...
const context={json:{email:'dev@m2m.local',profile:{name:'M2M'}},node:{Webhook:{json:{body:{id:42}}}},env:{API_URL:'http://localhost:3000'},workflow:{id:'workflow-1'},execution:{id:'execution-1'}};
...
describe('expression engine',()=>{
  ...
  it('rejects executable syntax',()=>expect(()=>validateExpression('$json.email.toUpperCase()')).toThrow());
});
